//wp_grid_builder/frontend/blocks/base.php
<?php
/**
 * Default blocks
 *
...
function wpgb_get_the_post_thumbnail_url( $size = 'full', $metadata = '', $empty = false ) {
	...
	if ( ! empty( $object ) && is_object( $object ) ) {
		...
		$metadata = get_post_meta($object->ID);

		$attributes =  $metadata['_product_attributes'];

		$unserialize = unserialize($attributes[0]);

		if(isset($unserialize['images'])) {
			$images = explode('|', $unserialize['images']['value']);
